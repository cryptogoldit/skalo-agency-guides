...
```mermaid
flowchart LR
    subgraph INTAKE["🔍 Fonti Dati"]
        A1[LinkedIn Pubblico]
        A2[Registri Camerali]
        A3[Directory Settore]
    end

    subgraph ENGINE["⚙️ Automated Lead Generation Engine"]
        B1[Scraping Controllato]
        B2[Data Enrichment\nEmail · Dimensione · Tecnologie]
        B3[AI Scoring\nClassificatore ML 0-100]
        B4{Score ≥ 65?}
    end

    subgraph CRM["📊 Skalo CRM & Sales OS"]
        C1[Pipeline Kanban]
        C2[Script Vendita AI]
        C3[Generatore Offerte PDF]
        C4[AI Sales Support]
    end

    subgraph OUTPUT["📈 Output Misurabili"]
        D1[Lead Qualificati]
        D2[Pipeline Ordinata]
        D3[Report 3 KPI]
    end

    subgraph VISIBILITY["🌐 Visibilità Digitale"]
        E1[Sito Next.js]
        E2[LinkedIn Content]
        E3[Advertising B2B]
    end
    ...
    style ENGINE fill:#1e1b4b,stroke:#6366f1,color:#c7d2fe
    style CRM fill:#1e1b4b,stroke:#6366f1,color:#c7d2fe
    style OUTPUT fill:#0f172a,stroke:#4f46e5,color:#a5b4fc
    style VISIBILITY fill:#0f172a,stroke:#4f46e5,color:#a5b4fc
    style SCARTO fill:#1f0a0a,stroke:#ef4444,color:#fca5a5
```
